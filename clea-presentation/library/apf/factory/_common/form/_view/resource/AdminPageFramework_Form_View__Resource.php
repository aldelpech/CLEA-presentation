...
class clea_presentation_AdminPageFramework_Form_View__Resource extends clea_presentation_AdminPageFramework_FrameworkUtility {
    public $oForm;
    ...
    private function _setHooks() {
        ...
        new clea_presentation_AdminPageFramework_Form_View__Resource__Head($this->oForm, 'wp_head');
    }
    private function _setAdminHooks() {
        ...
        new clea_presentation_AdminPageFramework_Form_View__Resource__Head($this->oForm, 'admin_head');
    }
    ...
    private function _getFormattedEnqueueScript($asEnqueue) {
        ...
        $_aEnqueueItem = $this->getAsArray($asEnqueue) + array('handle_id' => 'script_' . $this->oForm->aArguments['caller_id'] . '_' . $_iCallCount, 'src' => null, 'dependencies' => null, 'version' => null, 'in_footer' => false, 'translation' => null,);
    ...
    private function _getFormattedEnqueueStyle($asEnqueue) {
        static $_iCallCount = 1;
        $_aEnqueueItem = $this->getAsArray($asEnqueue) + array('handle_id' => 'style_' . $this->oForm->aArguments['caller_id'] . '_' . $_iCallCount, 'src' => null, 'dependencies' => null, 'version' => null, 'media' => null,);
        if (is_string($asEnqueue)) {
            $_aEnqueueItem['src'] = $asEnqueue;
        }
        $_aEnqueueItem['src'] = $this->getResolvedSRC($_aEnqueueItem['src']);
    ...
    public function _replyToAddStyle() {
        ...
        if ($_sCSSRules) {
            echo "<style type='text/css' id='internal-style-{$_sID}' class='clea-presentation-form-style'>" . $_sCSSRules . "</style>";
        }
        $_sIECSSRules = $this->_getFormattedInternalStyles($this->oForm->getResources('internal_styles_ie'));
        if ($_sIECSSRules) {
            echo "<!--[if IE]><style type='text/css' id='internal-style-ie-{$_sID}' class='clea-presentation-form-ie-style'>" . $_sIECSSRules . "</style><![endif]-->";
        }
    ...
    public function _replyToAddScript() {
        ...
        if ($_sScript) {
            $_sID = $this->sanitizeSlug(strtolower($this->oForm->aArguments['caller_id']));
            echo "<script type='text/javascript' id='internal-script-{$_sID}' class='clea-presentation-form-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
        }
