...
abstract class clea_presentation_AdminPageFramework_FieldType_Base extends clea_presentation_AdminPageFramework_Form_Utility {
    public $_sFieldSetType = '';
    public $aFieldTypeSlugs = array('default');
    protected $aDefaultKeys = array();
    protected static $_aDefaultKeys = array('value' => null, 'default' => null, 'repeatable' => false, 'sortable' => false, 'label' => '', 'delimiter' => '', 'before_input' => '', 'after_input' => '', 'before_label' => null, 'after_label' => null, 'before_field' => null, 'after_field' => null, 'label_min_width' => 0, 'before_fieldset' => null, 'after_fieldset' => null, 'field_id' => null, 'page_slug' => null, 'section_id' => null, 'before_fields' => null, 'after_fields' => null, 'attributes' => array('disabled' => null, 'class' => '', 'fieldrow' => array(), 'fieldset' => array(), 'fields' => array(), 'field' => array(),),);
    protected $oMsg;
    public function __construct($asClassName = 'admin_page_framework', $asFieldTypeSlug = null, $oMsg = null, $bAutoRegister = true) {
        ...
        $this->oMsg = $oMsg ? $oMsg : clea_presentation_AdminPageFramework_Message::getInstance();
        if ($bAutoRegister) {
    ...
    protected function getFieldOutput(array $aFieldset) {
        ...
        $_oFieldset = new clea_presentation_AdminPageFramework_Form_View___Fieldset($aFieldset, $_oCallerForm->aSavedData, $_oCallerForm->getFieldErrors(), $_oCallerForm->aFieldTypeDefinitions, $_oCallerForm->oMsg, $_oCallerForm->aCallbacks);
    ...
    protected function enqueueMediaUploader() {
        ...
        if (function_exists('wp_enqueue_media')) {
            new clea_presentation_AdminPageFramework_Form_View___Script_MediaUploader($this->oMsg);
        } else {
    ...
    protected function getLabelContainerAttributes(array $aField, $asClassAttributes, array $aAttributes = array()) {
        ...
        $aAttributes['style'] = $this->getStyleAttribute(array('min-width' => $aField['label_min_width'] ? $this->getLengthSanitized($aField['label_min_width']) : null,), $this->getElement($aAttributes, 'style'));
        return $this->getAttributes($aAttributes);
    }
}
abstract class clea_presentation_AdminPageFramework_FieldType extends clea_presentation_AdminPageFramework_FieldType_Base {
    public function _replyToFieldLoader() {
        $this->setUp();
    }
    public function _replyToGetScripts() {
...
class clea_presentation_AdminPageFramework_FieldType_color extends clea_presentation_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".repeatable .colorpicker {display: inline;}.clea-presentation-field-color .wp-picker-container {vertical-align: middle;}.clea-presentation-field-color .ui-widget-content {border: none;background: none;color: transparent;}.clea-presentation-field-color .ui-slider-vertical {width: inherit;height: auto;margin-top: -11px;}.clea-presentation-field-color .clea-presentation-repeatable-field-buttons {margin-top: 0;}.clea-presentation-field-color .wp-color-result {margin: 3px;}";
    }
    protected function getScripts() {
        $_aJSArray = json_encode($this->aFieldTypeSlugs);
        $_sDoubleQuote = '\"';
        return <<<JAVASCRIPTS
registerclea_presentation_AdminPageFrameworkColorPickerField = function( osTragetInput, aOptions ) {
    ...
    var _aDefaults = {
        ...
        change: function( event, ui ){
            jQuery( this ).trigger( 
                'clea-presentation_field_type_color_changed',
        ...
        clear: function( event, ui ) {
            jQuery( this ).trigger(
                'clea-presentation_field_type_color_cleared',
    ...
    jQuery().registerclea_presentation_AdminPageFrameworkCallbacks( {     
        added_repeatable_field: function( oClonedField, sFieldType, sFieldTagID, sCallType ) {
            ...
            oClonedField.find( 'input.input_color' ).each( function( iIterationIndex ) {
                ...
                registerclea_presentation_AdminPageFrameworkColorPickerField( _oNewColorInput );                
...
JAVASCRIPTS;
    ...
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div class='clea-presentation-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'clea-presentation-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
    }
...
jQuery( document ).ready( function(){
    registerclea_presentation_AdminPageFrameworkColorPickerField( '{$sInputID}' );
});            
JAVASCRIPTS;
